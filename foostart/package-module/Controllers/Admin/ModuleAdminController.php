<?php namespace Foostart\Module\Controllers\Admin;
...
class ModuleAdminController extends FooController {
    ...
    public function config(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-module.php'));
        $package_path = realpath(base_path('vendor/foostart/package-module'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-module-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-module.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-module'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/module-admin.php'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name module-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/module-admin-'.date('YmdHis',time()).'.php', $content);
            }
