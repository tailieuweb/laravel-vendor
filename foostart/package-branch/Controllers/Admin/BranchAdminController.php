<?php namespace Foostart\Branch\Controllers\Admin;
...
use Foostart\Branch\Validators\BranchValidator;
...
class BranchAdminController extends FooController {
    ...
    public function __construct(Request $request) {
        ...
        //set language files
        $this->plang_admin = 'branch-admin';
        $this->plang_front = 'branch-front';

        //package name
        $this->package_name = 'package-branch';
        $this->package_base_name = 'branch';

        //root routers
        $this->root_router = 'branch';
        ...
        $this->category_ref_name = 'admin/branch';
    ...
    public function index(Request $request) {
        ...
        if ($is_admin ) {
            ...
            return redirect()->route('branch.list', ['user_id' => $user['user_id']]);
    ...
    public function config(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-branch.php'));
        $package_path = realpath(base_path('vendor/foostart/package-branch'));
        ...
        if ($request->isMethod('branch') && $is_valid_request) {

            //create backup of current config
            file_put_contents($config_bakup.'/package-branch-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-branch.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-branch'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/branch-admin.php'));
        ...
        if ($request->isMethod('branch') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name branch-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/branch-admin-'.date('YmdHis',time()).'.php', $content);
            }
