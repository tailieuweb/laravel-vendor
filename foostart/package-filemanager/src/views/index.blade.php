    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('foostart.lfm.upload') }}" role='form' id='uploadForm' name='uploadForm'
                        ...
                        <div class="controls text-center">
                            <div class="input-group" style="width: 100%">
                                <a class="btn btn-primary"
                                   id="upload-button">{{ trans('package-filemanager::lfm.message-choose') }}</a>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default"
                        data-dismiss="modal">{{ trans('package-filemanager::lfm.btn-close') }}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    Dropzone.options.uploadForm = {
        ...
        acceptedFiles: "{{ lcfirst(Str::singular(request('type') ?: '')) == 'image' ? implode(',', config('lfm.valid_image_mimetypes')) : implode(',', config('lfm.valid_file_mimetypes')) }}",
        maxFilesize: ({{ lcfirst(Str::singular(request('type') ?: '')) == 'image' ? config('lfm.max_image_size') : config('lfm.max_file_size') }} / 1000)
    }
</script>
</body>
</html>
